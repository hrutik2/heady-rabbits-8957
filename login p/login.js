...
signupButton.addEventListener('click', () => {
  ...
  fetch('https://64889f370e2469c038fe1ba1.mockapi.io/users', {
    method: 'POST',
    headers: {
...
const paywise = document.getElementById("paywise");
const text = "Paywise ";
let currentIndex = 0;
...
function appendNextLetter() {
  ...
  if (currentIndex < text.length) {
    ...
    paywise.appendChild(span);

    span.getBoundingClientRect();
    span.style.opacity = "1";
    currentIndex++;
    setTimeout(appendNextLetter, 500); // Adjust the delay (in milliseconds) between each letter
  }
  if (currentIndex==text.length) {
    currentIndex=0
    paywise.innerText=``
